...
export default function CheckoutPage() {
    ...
    useEffect(() => {
        ...
        if ((session?.user as any)?.plan === "StreamVault+") {
            router.push("/");
            return;
        }
    }, [session, router]);

    if (!session || (session?.user as any)?.plan === "StreamVault+") {
        return null;
    }
        ...
        <div className="min-h-screen bg-[#0B0B0F] flex items-center justify-center relative p-6 pt-24 overflow-hidden">
            ...
            <div className="max-w-[1000px] w-full grid grid-cols-1 lg:grid-cols-2 gap-12 relative z-10">
                ...
                <div className="bg-[rgba(255,255,255,0.05)] backdrop-blur-[40px] border border-[rgba(255,255,255,0.1)] rounded-[32px] p-8 md:p-10 shadow-[0_20px_80px_rgba(0,0,0,0.5)]">
                    ...
                    <Button
                        disabled={true}
                        className="w-full h-14 bg-white/50 text-black text-[16px] font-bold rounded-full transition-transform hover:scale-[1.02] active:scale-95 disabled:opacity-70 disabled:hover:scale-100 shadow-[0_10px_30px_rgba(255,255,255,0.1)]"
                    >
                        Coming Soon
                    </Button>
                    ...
                    <div className="flex flex-col items-center justify-center gap-3 mt-6">
                        ...
                        <div className="flex items-center gap-4 text-white/30">
                            ...
                            <div className="flex items-center" style={{ marginLeft: '-4px' }}>
                                <div className="w-5 h-5 rounded-full bg-red-500/50 mix-blend-screen" style={{ marginRight: '-8px' }}></div>
                                <div className="w-5 h-5 rounded-full bg-orange-500/50 mix-blend-screen"></div>
                            </div>
